...
const VendorPage = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const getVendordata = async () => {
      ...
      const { address, area, opening_hour, closing_hour, name, id, rating } =
        vendor;
      ...
      for (const product of rawProducts) {
        ...
        try {
          ...
          url = await getDownloadURL(
            ref(
              storage,
              "gs://excess-d8b1e.appspot.com/product images/default_image.jpg"
            )
          );
        } finally {
      ...
      <View style={{ width: "100%", paddingTop: 16 }}>
        ...
        >
          ...
          <View style={{ alignSelf: "flex-start", width: 250, flex: 1 }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <AppText
                style={{ fontSize: 12, color: colorStyles[50], marginLeft: 8 }}
              >
                Opens {formatTime(vendor.opening_hour)} -{" "}
        ...
        >
          ...
          <View>
            ...
            <AppText style={{ fontSize: 12, color: colorStyles[50] }}>
              Ready in 5 minutes
            </AppText>
          </View>
        </View>
          ...
          if (section.tileType === "BigProducts") {
            return (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <BigProductCard product={item} cartHandler={cartHandler} />
                )}
              />
            );
          }
          return <ProductCard product={item} cartHandler={cartHandler} />;
        }}
